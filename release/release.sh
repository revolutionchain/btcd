...
PKG="github.com/qtumproject/btcd"
...
for i in $SYS; do
    ...
    env CGO_ENABLED=0 GOOS=$OS GOARCH=$ARCH GOARM=$ARM go build -v -trimpath -ldflags="-s -w -buildid=" github.com/qtumproject/btcd
    env CGO_ENABLED=0 GOOS=$OS GOARCH=$ARCH GOARM=$ARM go build -v -trimpath -ldflags="-s -w -buildid=" github.com/qtumproject/btcd/cmd/btcctl
    cd ..
