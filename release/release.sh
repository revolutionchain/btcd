...
PKG="github.com/alejoacosta74/btcd"
...
for i in $SYS; do
    ...
    env CGO_ENABLED=0 GOOS=$OS GOARCH=$ARCH GOARM=$ARM go build -v -trimpath -ldflags="-s -w -buildid=" github.com/alejoacosta74/btcd
    env CGO_ENABLED=0 GOOS=$OS GOARCH=$ARCH GOARM=$ARM go build -v -trimpath -ldflags="-s -w -buildid=" github.com/alejoacosta74/btcd/cmd/btcctl
    cd ..
